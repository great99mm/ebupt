...
fn job_json(r: sqlx::postgres::PgRow)->Value{ json!({"id":r.get::<Uuid,_>("id"),"source_item_id":r.get::<Uuid,_>("source_item_id"),"status":r.get::<String,_>("status"),"title":r.get::<String,_>("title"),"source":r.get::<String,_>("source"),"poster_url":r.try_get::<String,_>("poster_url").ok(),"created_at":r.get::<DateTime<Utc>,_>("created_at"),"archived_at":r.try_get::<DateTime<Utc>,_>("archived_at").ok(),"compare":r.try_get::<Value,_>("compare_json").ok()}) }
async fn my_claims(State(s): State<AppState>, headers: HeaderMap) -> impl IntoResponse { let u=match auth(&headers,&s.db).await{Ok(u)=>u,Err(c)=>return err(c,"unauthorized").into_response()}; let rows=sqlx::query("SELECT j.*, si.title, si.source, si.poster_url FROM maintenance_jobs j JOIN source_items si ON si.id=j.source_item_id WHERE j.user_id=$1 AND j.status NOT IN ('released','completed') ORDER BY j.created_at DESC").bind(u.id).fetch_all(&s.db).await.unwrap_or_default(); Json(json!({"jobs":rows.into_iter().map(job_json).collect::<Vec<_>>() })).into_response() }
async fn my_archives(State(s): State<AppState>, headers: HeaderMap) -> impl IntoResponse { let u=match auth(&headers,&s.db).await{Ok(u)=>u,Err(c)=>return err(c,"unauthorized").into_response()}; let rows=sqlx::query("SELECT j.*, si.title, si.source, si.poster_url FROM maintenance_jobs j JOIN source_items si ON si.id=j.source_item_id WHERE j.user_id=$1 AND j.status='completed' ORDER BY j.archived_at DESC NULLS LAST, j.created_at DESC").bind(u.id).fetch_all(&s.db).await.unwrap_or_default(); Json(json!({"jobs":rows.into_iter().map(job_json).collect::<Vec<_>>() })).into_response() }
...
async fn compare_job(State(s): State<AppState>, headers: HeaderMap, Path(id): Path<Uuid>) -> impl IntoResponse { let u=match auth(&headers,&s.db).await{Ok(u)=>u,Err(c)=>return err(c,"unauthorized").into_response()}; let row=sqlx::query("SELECT j.id,si.title FROM maintenance_jobs j JOIN source_items si ON si.id=j.source_item_id WHERE j.id=$1 AND j.user_id=$2 AND j.status='claimed'").bind(id).bind(u.id).fetch_optional(&s.db).await.unwrap(); let Some(r)=row else{return err(StatusCode::NOT_FOUND,"job not found").into_response()}; let title:String=r.get("title"); let emby=sqlx::query("SELECT metadata FROM source_items WHERE source='emby' AND title=$1 ORDER BY updated_at DESC LIMIT 1").bind(&title).fetch_optional(&s.db).await.unwrap(); let (found,metadata) = match emby { Some(row)=>(true,row.get::<Value,_>("metadata")), None=>(false,json!({})) }; let result=compare_from_metadata(&title,&metadata,found); let _=sqlx::query("UPDATE maintenance_jobs SET compare_json=$1,updated_at=now() WHERE id=$2 AND user_id=$3").bind(&result).bind(id).bind(u.id).execute(&s.db).await; Json(json!({"compare":result})).into_response() }
async fn archive_job(State(s): State<AppState>, headers: HeaderMap, Path(id): Path<Uuid>) -> impl IntoResponse { let u=match auth(&headers,&s.db).await{Ok(u)=>u,Err(c)=>return err(c,"unauthorized").into_response()}; let row=sqlx::query("SELECT compare_json FROM maintenance_jobs WHERE id=$1 AND user_id=$2").bind(id).bind(u.id).fetch_optional(&s.db).await.unwrap(); let Some(r)=row else{return err(StatusCode::NOT_FOUND,"job not found").into_response()}; let compare:Option<Value>=r.try_get("compare_json").ok(); let progress=compare.as_ref().and_then(|v|v.get("progress")).and_then(|v|v.as_f64()).unwrap_or(0.0); let missing=compare.as_ref().and_then(|v|v.get("missing_count")).and_then(|v|v.as_f64()).unwrap_or(f64::MAX); if progress<100.0 || missing>0.0{return err(StatusCode::CONFLICT,"compare incomplete").into_response()} let n=sqlx::query("UPDATE maintenance_jobs SET status='completed',archived_at=now(),updated_at=now() WHERE id=$1 AND user_id=$2 AND status='claimed'").bind(id).bind(u.id).execute(&s.db).await.unwrap().rows_affected(); if n==0{err(StatusCode::CONFLICT,"cannot archive").into_response()}else{Json(json!({"ok":true,"status":"completed"})).into_response()} }
